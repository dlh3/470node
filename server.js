...
function httpServ(req, resp)
{
    ...
    switch (urlPage)
    {
        ...
        case '/webChat':
            ...
            webChat(req, resp, urlParts['query']['id']);
            break;
        case '/webSay':
            var date = new Date();
            var newline = "["+ date.format("isoTime") + "] " + webUsers[urlParts['query']['id']].username + ': ' + urlParts['query']['msg'];
            // Send to all users
            chatSay(newline);
            // And log it
    ...
    if (urlPage != '/uploader' && urlPage != '/webChat')
    {
        // Page Headers
        resp.writeHead(status, headers);
        // send the output
...
function webLogin(nick)
{
    ...
    randNum = Math.floor(Math.random()*0x000010000);
    var webusr = new User(nick, "%");
    webusr.push(users);
    webUsers[randNum] = webusr;
    console.log(webusr.username+' connected via WebChat, '+ connections.length +' total.');
    chatSay('* ' + webusr.username + ' joined from WebChat!');
    return randNum;
}

// Sends all messages to the webchat (request, response, [optional] the randNum from the previous connection)
function webChat(req, res, randNum)
{
    //req.connection.setKeepAlive(true, 300000)
    if (!randNum)
    {
        res.end();
    }
    console.log("Adding webcon");
...
// The User Class
//
function User(uname, urank, ustatus, umsg, uconn)
{
    this.username = uname;
    this.rank = urank;
    this.status = ustatus;
